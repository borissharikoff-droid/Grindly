...
import { CHEST_DEFS, LOOT_ITEMS, MARKETPLACE_BLOCKED_ITEMS, POTION_MAX, type ChestType, getItemPerkDescription } from '../../lib/loot'
...
const FILTERS = [
  ...
  { id: 'consumables',  label: 'Consumables', icon: '⚗️' },
...
function slotMatchesFilter(slot: SlotEntry, fid: string): boolean {
  ...
  if (fid === 'consumables')  return item.slot === 'consumable' || item.slot === 'food'
  if (fid === 'boosts')       return ['xp_skill_boost', 'xp_global_boost', 'focus_boost', 'chest_drop_boost'].includes(item.perkType as string)
...
export function InventoryPage({ onBack, onNavigateFarm }: { onBack: () => void; onNavigateFarm?: () => void }) {
  ...
  type FilterById = 'all' | 'gear' | 'consumables' | 'boosts' | 'resources' | 'chests' | 'cosmetic' | 'garden'
  const [filterBy, setFilterByRaw] = useState<FilterById>(() => {
    try {
      ...
      if (saved === 'potions' || saved === 'food') return 'consumables'
      if (saved === 'xp' || saved === 'drops') return 'boosts'
  ...
  return (
    ...
    >
      ...
      <div className="rounded-card border border-white/[0.08] bg-surface-2/80 p-3 space-y-2.5">
        ...
        {slots.length === 0 ? (
          <EmptyState
            icon="🎒"
            title="No loot yet"
            description="Grind sessions, open chests, harvest crops, and defeat bosses to fill your inventory."
          />
      ...
      {/* ── Item inspect modal (shared component) ── */}
      {inspectSlot?.kind === 'item' && (
        <ItemInspectModal
          item={inspectItem}
          locked={inBattle}
          onSell={
            inspectSlot && !MARKETPLACE_BLOCKED_ITEMS.includes(inspectSlot.itemId) && (!inspectSlot.equipped || inspectSlot.quantity > 1)
      ...
      {/* ── Seed / chest / pending inspect modal ── */}
      {inspectSlot && inspectSlot.kind !== 'item' &&
              ...
              <div className="flex-1 min-w-0 flex flex-col p-3.5 gap-0">
                ...
                <div className="pr-6">
                  ...
                  <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                    ...
                    {inspectSlot.kind === 'seed' && (
                      <span className="text-micro px-1.5 py-0.5 rounded border border-white/15 text-gray-400 font-mono uppercase tracking-wide">Seed</span>
                    )}
                  </div>
                </div>
