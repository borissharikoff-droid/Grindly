  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="flex items-center gap-1.5">
        <span className="text-[9px] text-gray-500 font-mono">Food <span className="text-gray-700">(1/run)</span>:</span>
        {slots.map((slot, idx) => {
      ...
      {pickerIdx !== null && (
        <div className="rounded-xl border border-white/[0.08] bg-white/[0.03] overflow-hidden">
          {availableFood.length === 0 ? (
            <p className="text-[9px] text-gray-600 font-mono text-center py-2">No food in inventory</p>
          ) : (
            availableFood.map((f) => {
              const owned = ownedItems[f.id] ?? 0
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 flex items-center gap-1.5 flex-wrap">
                    ...
                    {f.effect.buffDurationSec && (f.effect.buffAtk || f.effect.buffDef || f.effect.buffRegen) && (
                      <span className="text-[8px] font-mono text-gray-600">{f.effect.buffDurationSec}s</span>
                    )}
                  </div>
                  <span className="text-[9px] font-mono text-gray-500 shrink-0">×{owned - usedOther}</span>
                </button>
              )
            })
          )}
